...
## Files
- `VOC 登入.dc.html` — 登入頁
- `VOC Dashboard.dc.html` — 首頁儀表板（廠區導覽 + 異常摘要 + 手風琴明細）
...
5. 右上角 4 個工具按鈕（派送名單/規格維護/隔離申請/簽核）目前開啟一個佔位 modal，文案說明落地時應改為 `<dialog>` + 局部載入掛回既有表單，**不要用 Bootstrap Modal**
...
## 尚未包含 / 待確認
- `VOC Wireframes.dc.html`（探索階段的手繪風線框稿）**未包含在本次交付**，已被上述三個高保真畫面取代，可忽略
- 歷史曲線頁與儀表板目前都是前端模擬資料，需與後端資料表（廠棟、監測項目、門檻值、歷史時序）對接
- 儀表板右上角 4 個工具按鈕（派送名單/規格維護/隔離申請/簽核）目前僅為佔位 modal，功能落地時需求另行確認
